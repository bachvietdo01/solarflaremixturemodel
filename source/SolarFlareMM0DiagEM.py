...
class SolarFlareMM0DiagEM:
    def __init__(self, X, y, K, sigma20 = None, pi0 = None, mu0 = None, Sigma_diag0 = None, beta0 = None,
                 debug_r = None, debug_beta = None, debug_pi = None, debug_mu = None, 
                 debug_Sigma_diag = None, debug_sigma2 = None):
        self.X = X # xovariates
        self.y = y # response
        ...
        if Sigma_diag0 is not None:
            self.Sigma_diag = Sigma_diag0
        else:
            for k in range(K):
                self.Sigma_diag[k,] = np.ones(D)
        ...
        if debug_Sigma_diag is not None:
            self.Sigma_diag = debug_Sigma_diag
            self.debug_Sigma_diag = debug_Sigma_diag
    ...
    def E_step(self):
        ...
        for k in range(self.K):            
            for i in range(self.N):
                self.r[i,k] = np.log(self.pi[k]) + dist.multivariate_normal.logpdf(self.X[i,], 
                                     mean=self.mu[k,], cov= np.diag(self.Sigma_diag[k, ])) + \
                              dist.multivariate_normal.logpdf(self.y[i], 
    ...
    def M_step(self):
        ...
        for k in range(K):         
            ...
            for n in range(N):
                ...
                for p in range(D):
                    Sigma_diag_hat[k,p] += self.r[n,k] * np.square(self.X[n,p] - mu_hat[k,p])
